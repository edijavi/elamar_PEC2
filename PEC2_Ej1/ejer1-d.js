...
*/
    ...
    */
    ...
    para poder tener disponible el valor de un objeto promesa  y poder usarlo dentro try catch.
    */
        try {
            let  promise = await Promise.all([result]);
            /* 
            Se define let promise en dónde se guarda el resultado del método Promise.all([]) 
            el cual se le puede pasar múltiples promesas y manejará multiples llamadas a la vez
            y agrupa su resultado en uno sólo en forma de array
            */ 
            onSuccess(promise[0]);
        } catch (error) {
            onError(error);
